# -*- coding: utf-8 -*-
"""Juliawati Hutagaol_119140016_TUGAS AKHIR wine quality.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1YgDhwgmGOdwIUjy_mvCS3VfIXvZpUJK_
...
df = pd.read_csv("/content/drive/MyDrive/Dataset/winequality.csv")
...
print("Jumlah baris sebelum menghapus data duplikat:", len(df))

# Menghapus data duplikat berdasarkan semua kolom
df.drop_duplicates(inplace=True)

# Menampilkan jumlah baris setelah menghapus data duplikat
print("Jumlah baris setelah menghapus data duplikat:", len(df))

# Mengganti nilai dalam kolom 'type'
df['type'] = df['type'].replace({'white': 1, 'red': 0})
df

cols=df.columns
cols=['fixed acidity','volatile acidity','citric acid','residual sugar','chlorides','free sulfur dioxide','total sulfur dioxide','density','pH','sulphates','alcohol','quality','type']
...
df.columns = ['fixed acidity', 'volatile acidity', 'citric acid', 'residual sugar', 'chlorides', 'free sulfur dioxide', 'total sulfur dioxide', 'density', 'pH', 'sulphates','alcohol','quality','type']
...
# Normalisasi data
scaling =  ['fixed acidity', 'volatile acidity', 'citric acid', 'residual sugar', 'chlorides', 'free sulfur dioxide', 'total sulfur dioxide', 'density', 'pH', 'sulphates','alcohol','quality']
...
rf1 = RandomForestClassifier(n_estimators=12, max_features="sqrt", random_state=42)
...
sns.countplot(x="type", data=df_scaled)
...
df_oversampled = pd.concat([X_resampled, y_resampled.rename('type')], axis=1)
# Menampilkan jumlah kelas setelah oversampling
class_counts = df_oversampled['type'].value_counts()
print(class_counts)

# Menampilkan snsplot
sns.countplot(x='type', data=df_oversampled)

"""Hyperparamameter Tuning menggunakan GridSearchCV"""

param_grid = {
    'n_estimators': [12,100,200,300],
...
rf2 = RandomForestClassifier(n_estimators=200, max_features='auto', max_depth=None, criterion='entropy', random_state=42)
...
df_Balanced = pd.concat([X_oversampled, y_oversampled.rename('type')], axis=1)
# Menampilkan jumlah kelas setelah oversampling
class_counts = df_Balanced['type'].value_counts()
print(class_counts)

# Menampilkan snsplot
sns.countplot(x='type', data=df_Balanced)

"""Hyperparameter Tuning Random Forest dengan GridSearchCV pada smote"""
